...
# operating system:  Linux 4.13.0-41-generic x86_64 [ELF] (4.13.0)
# libc:	(6.23.0)
# binutils:	(226)
...
#        DEPLIBS = $(DEPXMLIB) $(DEPXTOOLLIB) $(DEPXPLIB) $(DEPXLIB)
        DEPLIBS =
# LOCAL_LIBRARIES = $(XMLIB) $(XTOOLLIB) $(XPLIB) $(XLIB) $(XMEXTRA_LIBS) -lXpm
LOCAL_LIBRARIES =  -lXm -lXt -lX11 -lXpm
XPLIBSRC	=
...
# was MComplexProgramTarget(minfo, $(DEPLIBS), $(LOCAL_LIBRARIES))
...
all:: minfo.$(MANNEWSUFFIX)

minfo.$(MANNEWSUFFIX): minfo.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname minfo` && \
	   $(LN) `basename minfo.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < minfo.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) minfo.$(MANNEWSUFFIX)

install.man:: minfo.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) minfo.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/minfo.$(MANSUFFIX)
